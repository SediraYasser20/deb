...
if ($action == 'create') {
	...
	if ($action != 'prerelance' && $action != 'presend' && $action != 'valid' && $action != 'editline') {
		...
		if (empty($reshook)) {
			...
			$in_group_5 = false;
			if (!empty($user->id)) { // Ensure user is logged in
			    $sql_group = "SELECT COUNT(*) as count FROM ".MAIN_DB_PREFIX."usergroup_user WHERE fk_user = ".((int) $user->id)." AND fk_usergroup = 5"; // Cast user->id to int
			    $resql_group = $db->query($sql_group);
			    if ($resql_group) {
			        $obj_group = $db->fetch_object($resql_group);
			        if ($obj_group && $obj_group->count > 0) {
			            $in_group_5 = true;
			        }
			        $db->free($resql_group); // Free the result
			    } else {
			        dol_print_error($db); // Log error if query fails
			    }
			}
			$can_create_credit_note_permission = ($user->admin || $in_group_5); // User is admin OR in group 5

			// Condition to show the button:
			// - Invoice type is Standard, Deposit (if not just payment), or Proforma
			// - Invoice is paid (Validated and paye=1, OR Closed)
			// - User has general permission to create invoices/notes ($usercancreate)
			// - User has specific permission to create credit notes ($can_create_credit_note_permission)
			if (($object->type == Facture::TYPE_STANDARD || ($object->type == Facture::TYPE_DEPOSIT && !getDolGlobalString('FACTURE_DEPOSITS_ARE_JUST_PAYMENTS')) || $object->type == Facture::TYPE_PROFORMA)
				&& (($object->status == Facture::STATUS_VALIDATED && $object->paye == 1) || $object->status == Facture::STATUS_CLOSED)
				&& $usercancreate && $can_create_credit_note_permission
			) {
				if (!$objectidnext) { // Not already replaced
				...
				// Optional: Add an else here to show a disabled button with a tooltip if $objectidnext is true,
				// but the original code just doesn't show it, so I'll stick to that.
			}
			// End Create a credit note

			// For situation invoice with excess received
			// Note: The logic for situation invoices might also need adjustment based on the same new permission model.
			// However, the original request was focused on the primary "Create a credit note" button.
			// If this also needs changing, it should be a separate, specific instruction.
			// For now, I'll leave this second block as it was, or you can provide specific replacement for it.
			if ($object->status > Facture::STATUS_DRAFT
				&& $object->isSituationInvoice()
				&& ($object->total_ttc - $totalpaid - $totalcreditnotes - $totaldeposits) > 0
				&& $usercancreate // This still uses the general $usercancreate. Modify if needed.
				&& !$objectidnext
				&& $object->is_last_in_cycle()
			...
			// Clone
  if (($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_PROFORMA) && $usercancreate && $user->admin) { // Restrict Clone to admin
            unset($params['attr']['title']);
			...
			if (($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_PROFORMA) && $object->status == 0 && $usercancreate && $user->admin) {
