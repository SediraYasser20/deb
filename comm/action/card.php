...
// Check if status is "Finished" (100%)
if ($percent >= 100 && !$user->admin) {
    // Display read-only status for non-admins when Finished
    print $formactions->form_select_status_action('formaction', $percent, 0, 'complete', 0, 0, 'maxwidth200');
} else {
    // Display editable dropdown for admins or when not Finished
    print $formactions->form_select_status_action('formaction', $percent, 1, 'complete', 0, 0, 'maxwidth200');
}
print '</td></tr>';
...
if ($user->admin) {
    ...
    // Display the note as read-only for non-admins
    $note_content = !empty($object->note_private) ? dol_nl2br(dol_escape_htmltag($object->note_private)) : '<span class="opacitymedium">'.$langs->trans("NoDescription").'</span>';
    print '<div class="note-public">'.$note_content.'</div>';
}
print '</td></tr>';
		// Other attributes
		$parameters = array();
...
if (empty($reshook)) {
    if ($action != 'edit') {
        ...
        $isAssignedUser = !empty($object->userassigned) && isset($object->userassigned[$user->id]);
        $isFinished = ($object->percentage >= 100 || $object->status == 9); // Adjust status check as needed

        // Show button if:
        // 1. Task is unfinished AND user is assigned, OR
        // 2. User is admin (regardless of status)
        if (($isAssignedUser && !$isFinished) || $user->admin) {
            $showModifyButton = true;
        }

        if ($showModifyButton) {
            print '<div class="inline-block divButAction">'
                . '<a class="butAction" href="card.php?action=edit&token='.newToken().'&id='.$object->id.'">'
                . $langs->trans("Modify")
                . '</a></div>';
        } else {
            print '<div class="inline-block divButAction">'
                . '<a class="butActionRefused classfortooltip" href="#" title="'.$langs->trans("NotAllowed").'">'
                . $langs->trans("Modify")
                . '</a></div>';
        }
        ...
        if ($user->hasRight('agenda', 'allactions', 'create') ||
           (($object->authorid == $user->id || $object->userownerid == $user->id)
            ...
            print '<div class="inline-block divButAction">'
                . '<a class="butAction" href="card.php?action=clone&object='.$object->element.'&id='.$object->id.'">'
                . $langs->trans("ToClone")
                . '</a></div>';
        } else {
            print '<div class="inline-block divButAction">'
        ...
        if ($user->hasRight('agenda', 'allactions', 'delete') ||
           (($object->authorid == $user->id || $object->userownerid == $user->id)
            ...
            print '<div class="inline-block divButAction">'
                . '<a class="butActionDelete" href="card.php?action=delete&token='.newToken().'&id='.$object->id.'">'
                . $langs->trans("Delete")
                . '</a></div>';
        } else {
            print '<div class="inline-block divButAction">'
...
print '</div>';
    ...
    if (!getDolGlobalString('AGENDA_DISABLE_BUILDDOC')) {
        ...
        print '<a name="builddoc"></a>'; // anchor
        ...
        print $formfile->showdocuments(
            'actions', $object->id, $filedir, $urlsource,
            $genallowed, $delallowed,
            '', 0, 0, 0, 0, 0, '', '', '', $langs->getDefaultLang()
        );
